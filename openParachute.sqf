...
if (!alive player) exitWith {};if (vehicle player != player) exitWith {};
private ["_wait", "_pos", "_para"];
// some aircraft blow up on contact with parachutes, so we have to make sure no one is close

waitUntil {
    sleep 0.1;
    ({player distance _x < 10 max (sizeOf typeOf _x)} count (player nearEntities ["Helicopter_Base_F", 20]) == 0)
};
...
if (_pos select 2 < 20) then{ _para = createVehicle ["NonSteerable_Parachute_F", _pos, [], 0, "FLY"];
