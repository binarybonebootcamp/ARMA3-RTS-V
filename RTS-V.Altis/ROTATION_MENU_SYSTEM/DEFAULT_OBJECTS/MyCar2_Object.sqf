...
_obj = "C_Offroad_01_F" createVehicle getPosATL player; //put an object here, such as a soldier or vehicle
MY_BIG_CAR = _obj;

_text = "EXPAND"; //text you want to display on the icon if this menu has priority

_modelPosition = [0,0,1]; //where, in model space, you want to display the menu icon
...
_menuItems = 
	[
		["Detonate", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", {(_this select 0) setDammage 1;}, [], _distance, [[], {(alive (_this select 0))}], RTMS_DEFAULT_SUB_ICON]
	];
